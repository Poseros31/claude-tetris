...
- Las **7 piezas estándar** (I, O, T, S, Z, J, L) con colores diferenciados, más una **pieza reto**: la **tuerca** (3×3 con hueco central).
...
- **Modelo del tablero**: una matriz `ROWS × COLS` donde cada celda guarda `0` (vacía), un índice de color (1–8) o `-1` (hueco de la tuerca: sólido a efectos de colisión y líneas, pero invisible).
- **Piezas**: definidas como matrices cuadradas. Para rotar se calcula la transposición + reverso de filas (`rotateCW`). La 8ª pieza es la **tuerca** (`N`), un bloque 3×3 con un `-1` en el centro: se apoya como una pieza maciza (nada puede colarse en su hueco), pero al comprobar líneas completas cuenta como celda rellena.
